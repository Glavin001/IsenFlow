...
test.describe('demo 04 — building flood', () => {
  test('south-edge tide rises, water enters via door, north wall holds', async ({
    ...
    // Tide depth at the breach cells (the only south-edge cells that have
    // a bed level of 0 and therefore can hold water — the rest of the row
    // is a 4 m wall) should track the inflow boundary's target value.
    const breachEdgeMean = await page.evaluate(
    ...
    expect(breachEdgeMean, `breach-edge mean h vs tide ${tideH.toFixed(2)}`).toBeGreaterThan(tideH * 0.5);
